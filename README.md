...
I would also considered adding a transactions array and a blockchain timestamps array to the Watchlist entity, meaning that every transaction hash and timestamp from one of the addresses in a watchlist would be added to it,  while being mindful of the fact that this could cause a [large array](https://thegraph.com/blog/improve-subgraph-performance-avoiding-large-arrays/) problem. When one watchlist matches everything that isn't feasible, so that would be removed.
